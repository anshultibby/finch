  ...
  const renderContentWithPlots = () => {
    ...
    parts.forEach((part, idx) => {
      ...
      if (fileMatch) {
        ...
        if (fileResource && chatId) {
          if (isImage) {
            // Embed image inline
            const imageUrl = `/api/chat-files/${chatId}/download/${filename}`;
            elements.push(
              <div key={`image-${idx}`} className="not-prose my-4">
                <img 
                  src={imageUrl} 
                  alt={filename}
                  className="max-w-full h-auto rounded-lg shadow-md border border-gray-200 cursor-pointer hover:shadow-lg transition-shadow"
                  onClick={() => onFileClick && onFileClick(filename, chatId)}
            ...
            // Render non-image files as clickable link with file icon
            elements.push(
              <button
                key={`file-${idx}`}
