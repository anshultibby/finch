...
import { INVESTOR_PERSONAS } from '@/lib/aiPrompts';
...
export default function ChatPage({
  ...
  const activePersona = activePersonaId ? INVESTOR_PERSONAS.find(p => p.id === activePersonaId) : null;
        ...
        <div className="flex items-center gap-2.5">
          {activePersona ? (
            <div
              ...
              style={{ background: activePersona.gradientStyle }}
            >
              <span className="text-[11px] font-bold text-white">{activePersona.initial}</span>
            </div>
          ) : (
          ...
          <div>
            <div className="text-sm font-semibold text-gray-900">
              {activePersona ? activePersona.name : 'AI Assistant'}
            </div>
            <div className="text-[11px] text-gray-400">
              {activePersona ? activePersona.tagline : currentChatId ? 'Continuing conversation' : null}
            </div>
          </div>
        </div>
        <button
