...
const ChatHistorySidebar = forwardRef<ChatHistorySidebarRef, ChatHistorySidebarProps>(({
  ...
  const renderChatItem = (chat: Chat) => {
    ...
    if (isCollapsed) {
      return (
        <button
          ...
          className={`w-10 h-10 flex items-center justify-center rounded-lg transition-all duration-150 text-lg touch-manipulation ${
            isActive
    ...
    return (
      <button
        ...
        onClick={() => onSelectChat(chat.chat_id)}
        title={getChatTitle(chat)}
        className={`w-full text-left rounded-lg transition-all duration-150 px-3 py-2.5 group touch-manipulation ${
          isActive
            ? 'bg-white shadow-sm border border-gray-200'
            : 'hover:bg-gray-100 active:bg-gray-200'
        }`}
  ...
  return (
    <>
      ...
      <div
        className={`h-full bg-gray-50 flex-shrink-0 transition-all duration-200 ease-out border-r border-gray-200 flex flex-col safe-area-left
          ${isCollapsed 
            ? 'w-[60px] md:w-[60px]' 
            : 'w-64 md:w-64'
          }
      ...
      >
        {/* Header */}
        <div className={`flex items-center p-3 safe-area-top ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!isCollapsed && (
            <span className="text-sm font-semibold text-gray-700">Chats</span>
          )}
          <button
            onClick={onToggle}
            className="p-1.5 hover:bg-gray-200 active:bg-gray-300 rounded-md transition-colors touch-manipulation"
      ...
      <div className={`px-2 pb-3 ${isCollapsed ? 'px-[10px]' : ''}`}>
        <button
          onClick={onNewChat}
          className={`flex items-center gap-2 w-full bg-white hover:bg-gray-100 active:bg-gray-200 border border-gray-200 rounded-lg transition-all text-sm font-medium text-gray-700 touch-manipulation ${
