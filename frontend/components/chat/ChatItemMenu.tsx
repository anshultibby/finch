...
interface ChatItemMenuProps {
  ...
  onShareChange: (chatId: string, isPublic: boolean, shareToken: string | null) => void;
  /** Extra classes for the kebab button (e.g. visibility-on-hover). */
...
export default function ChatItemMenu({
  ...
  const openShare = async () => {
    ...
    if (!isPublic || !shareToken) {
      ...
      try {
        ...
        onShareChange(chatId, res.is_public, res.share_token);
      } catch (err) {
  ...
  return (
    <div ref={menuRef} className="relative flex-shrink-0" onClick={stop}>
      <button
        onClick={(e) => { stop(e); setOpen(v => !v); }}
        className={`p-1 rounded-md hover:bg-gray-200 text-gray-400 hover:text-gray-700 transition-colors ${buttonClassName}`}
        title="Chat options"
      ...
      {modal === 'rename' && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/30" onClick={() => setModal('none')}>
          <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-[90%] max-w-sm p-4" onClick={stop}>
      ...
      {modal === 'share' && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/30" onClick={() => setModal('none')}>
          <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-[90%] max-w-md p-4" onClick={stop}>
