...
import { ArrowUpRight, ArrowDownRight, CheckCircle, XCircle, Clock, AlertCircle } from 'lucide-react';
...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  pending_approval: { label: 'Pending', color: 'text-blue-500' },
  approved: { label: 'Approved', color: 'text-emerald-600' },
  rejected: { label: 'Rejected', color: 'text-red-500' },
  expired: { label: 'Expired', color: 'text-gray-400' },
};

export default function BotTradesPanel({ userId, botId, refreshKey, onBack }: BotTradesPanelProps) {
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[11px] text-gray-400">
                          ...
                          {t.side && <span className={`px-1 py-0.5 rounded text-[10px] font-semibold ${t.side === 'yes' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>{t.side}</span>}
