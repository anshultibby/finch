...
export default function BotView({ botId }: BotViewProps) {
  ...
  const fetchChats = useCallback(async () => {
    ...
    try {
      ...
      setChats(data);
      // If no active chat and there are chats, select the first one
  ...
  // Auto-create first chat for new bots
  useEffect(() => {
    if (!user || !bot || chats.length > 0 || activeChatId) return;
    ...
    createFirstChat();
  }, [user, bot, chats.length, activeChatId, botId]);
    ...
    <div className="flex flex-col h-dvh bg-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 shrink-0">
          <BotSidebar bot={bot} onClosePosition={handleClosePosition} />
        </div>
      </div>
    </div>
  );
