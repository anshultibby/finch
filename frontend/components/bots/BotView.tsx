...
export default function BotView({ botId }: BotViewProps) {
  ...
  return (
    <div className="flex h-dvh bg-white">
      {/* Left sidebar */}
      {sidebarOpen ? (
        <div className="shrink-0 w-56 flex flex-col border-r border-gray-200 transition-all duration-200">
          {/* Sidebar header */}
        ...
        /* Collapsed icon rail — like ChatGPT */
        <div className="shrink-0 w-12 flex flex-col items-center border-r border-gray-200 py-3 gap-1">
          {[
            ...
            { label: 'Current chat', onClick: () => setActivePanel('chat'), icon: <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />, active: activePanel === 'chat', spinning: true, dividerAfter: true },
            ...
            <React.Fragment key={item.label}>
              <button
                ...
                className={`relative group p-2 rounded-lg transition-colors ${item.active ? 'bg-gray-200 text-gray-900' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
              >
                {item.spinning && item.active && (
                  <span className="absolute inset-0 rounded-lg border-2 border-blue-400 animate-ping opacity-20" />
                )}
                <svg className="w-[18px] h-[18px] relative" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">{item.icon}</svg>
                <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-medium text-white bg-gray-800 rounded-md whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50 shadow-lg">
                  {item.label}
                </span>
              </button>
              {item.dividerAfter && <div className="w-6 border-t border-gray-200 my-1" />}
            </React.Fragment>
          ))}
        </div>
      )}
      ...
      <div className="flex-1 min-w-0 flex flex-col relative">
        ...
        <div className="flex items-center gap-2.5 px-4 py-2.5 shrink-0">
          <span className="text-sm font-semibold text-gray-900">Finch</span>
          <div className="flex-1" />
          {bot.capital_balance != null && bot.platform !== 'research' && (
            <button
              onClick={() => setShowCapitalModal(true)}
              className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors"
            >
              <span className="text-[11px] font-semibold text-gray-700 tabular-nums">${bot.capital_balance.toFixed(2)}</span>
            </button>
          )}
          <button onClick={() => setShowApiKeys(true)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Settings">
