...
# Vol risk premium: IV vs realized vol
df["vrp"] = df["iv30d"] - df["orHv20d"]
print(f"Avg vol risk premium: {df['vrp'].mean():.1%}")

# Earnings trade signals
pre_earn = df[df["nextErn"].notna() & (pd.to_datetime(df["nextErn"]) - df["tradeDate"]).dt.days.between(1, 5)]
print(f"{len(pre_earn)} days within 5 days of earnings")
```

## Common Workflows

### Is IV High or Low? (Should I Buy or Sell Premium?)

```python
s = get_summary("TSLA")
ivr = get_iv_rank("TSLA")
latest = ivr["history"][-1]

if latest["ivRank1y"] > 50:
    print(f"IV rank {latest['ivRank1y']} — elevated. Favor selling strategies (iron condors, credit spreads).")
else:
    print(f"IV rank {latest['ivRank1y']} — low. Favor buying strategies (long straddles, debit spreads).")
```

### Find the Best Expiration for a Covered Call

```python
chain = get_options_chain("AAPL")
# 30-45 DTE, ~0.30 delta (OTM call)
candidates = [s for s in chain["strikes"] if 30 <= s["dte"] <= 45 and 0.25 <= s.get("delta", 0) <= 0.35]
candidates.sort(key=lambda s: s["callMidIv"], reverse=True)  # highest premium first
for c in candidates[:5]:
    mid = (c["callBidPrice"] + c["callAskPrice"]) / 2
    print(f"{c['expirDate']} {c['strike']}C: IV={c['callMidIv']:.1%} mid=${mid:.2f} delta={c['delta']:.2f}")
```

### Pre-Earnings Vol Analysis

```python
import pandas as pd
metrics = get_historical_metrics("NFLX", from_date="2020-01-01")
df = pd.DataFrame(metrics["history"])
df["tradeDate"] = pd.to_datetime(df["tradeDate"])

# Implied vs actual earnings moves
earn_rows = df[df["nextErn"].notna()].copy()
print("Implied earnings move history:")
print(earn_rows[["tradeDate", "impliedEarningsMove", "iv30d", "orHv20d"]].tail(10).to_string(index=False))
```

### Vol Term Structure & Contango

```python
s = get_summary("SPY")
ivs = [s.get(f"iv{t}d") or s.get(f"iv{t}") for t in ["10", "20", "30", "60", "90"]]
tenors = ["10d", "20d", "30d", "60d", "90d"]
for t, iv in zip(tenors, ivs):
    if iv:
        print(f"  {t}: {iv:.1%}")

# Contango > 0 means normal upward-sloping term structure
# Contango < 0 means backwardation (near-term fear)
print(f"Contango: {s.get('contango', 0):.3f}")
```

### Multi-Ticker IV Comparison

```python
for ticker in ["AAPL", "MSFT", "NVDA", "TSLA", "META"]:
    s = get_summary(ticker)
    if "error" not in s:
        print(f"{ticker}: 30d IV={s.get('iv30d', 0):.1%}  contango={s.get('contango', 0):.3f}")
```
