...
from routes import chat_router, snaptrade_router, robinhood_router, resources_router, chat_files_router, api_keys_router, credits_router, market_router, watchlist_router, push_router, analysis_router, bot_store_router, account_router, trades_router, apple_notifications_router, casparser_router, widget_router
...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "Authorization", "X-Admin-Secret", "X-User-ID", "Stripe-Signature"],
)
...
async def monitor_connection_pool():
    ...
    while True:
        try:
            ...
            if usage_percent > 80:
                logger.warning(
                    f"High DB pool usage: {pool_status['checked_out']}/{pool_status['total']} "
...
async def startup_event():
    ...
    if storage_service.is_available():
        ...
        if storage_service.ensure_bucket_exists():
            ...
            logger.warning("Failed to initialize Supabase Storage bucket")
    else:
