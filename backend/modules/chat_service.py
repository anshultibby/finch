...
class ChatService:
    ...
    async def get_user_chats(self, user_id: str, limit: int = 50, offset: int = 0, search: str | None = None, source: str = "user", db=None) -> List[dict]:
        async def _run(db):
            return await chat_async.get_user_chats_with_preview(db, user_id, limit, offset=offset, search=search, source=source)
        return await self._with_db(db, _run)
