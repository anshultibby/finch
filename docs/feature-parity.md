...
| Trade feedback (review a recent trade) | ✅ | ✅ | `GET /trades/recent` (unified SnapTrade `Transaction` + Robinhood filled orders, ranked recency + $ size); home list, tap seeds a chat that critiques the trade + suggests alternatives (`page_context.source="trade_feedback"`). Realized-P&L ranking deferred |
