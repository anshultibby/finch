...
| Strategy library (explore/adopt playbooks) | ✅ | ✅ | Agent-driven via the `strategy_library` skill: home "Explore strategies" entry seeds a chat → skill lists starter playbooks (`services/strategy_starters.py`) + the user's saved ones (`finch_api.list_strategies` → `GET /strategies`), explains one, and `adopt_strategy` persists it (`strategies` table, migration 098) + binds it to the goal (`UserGoal.config.strategy`, surfaced in the `<mission>` block). Distills new ones from Reddit/pasted playbooks via `strategy_distiller`. Gallery/publish/clone deferred (§4) |
